...
export function VideoStudio({ onLessonReady }) {
  ...
  const submitUpload = async (event) => {
    ...
    try {
      ...
      if (!data.lessonId) throw new Error(data.error || 'Upload failed');
      setLessons((prev) => [{ lessonId: data.lessonId, title: upForm.title, source: 'upload', status: 'uploading' }, ...prev]);
      setFile(null);
        ...
        <div className="p-7">
          {tab === 'youtube' && (
            <form onSubmit={submitYT} className="flex flex-col gap-[18px]">
              ...
              <SubmitRow busy={busy} busyText="Submitting..." icon="rocket" text="Create Lesson">
          ...
          {tab === 'upload' && (
            <form onSubmit={submitUpload} className="flex flex-col gap-[18px]">
              <Field label="Video File * (MP4, WebM, MOV - max 500 MB)">
                <div
                  role="button"
                  tabIndex={0}
              ...
              {err && <p className="text-[13px] text-red-400"><AppIcon name="alert" size={14} /> {err}</p>}
              <SubmitRow busy={busy || !file} busyText="Uploading..." icon="upload" text="Upload & Process">
...
function SubmitRow({ busy, busyText, icon, text, children }) {
  return (
    <div className="flex flex-wrap items-center gap-3.5">
      <button type="submit" disabled={busy} className="inline-flex items-center gap-2 rounded-[10px] bg-accent px-7 py-3 text-sm font-bold text-white disabled:cursor-not-allowed disabled:bg-[#333] disabled:opacity-60">
